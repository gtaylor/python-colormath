...
class DeltaETestCase(unittest.TestCase):
    ...
    def test_cie2000_accuracy(self):
        ...
        self.assertAlmostEqual(
            ...
            expected,
            3,
            "DeltaE CIE2000 formula error. "
            "Got %.3f, expected %.3f (diff: %.3f)."
    ...
    def test_cmc_accuracy(self):
        ...
        self.assertAlmostEqual(
            ...
            expected,
            3,
            "DeltaE CMC Atan test formula error. "
            "Got %.3f, expected %.3f (diff: %.3f)."
            % (result, expected, result - expected),
        )
    ...
    def test_cie1994_accuracy_graphic_arts(self):
        ...
        self.assertAlmostEqual(
            ...
            expected,
            3,
            "DeltaE CIE1994 (graphic arts) formula error. "
            "Got %.3f, expected %.3f (diff: %.3f)."
            % (result, expected, result - expected),
        )

    def test_cie1994_accuracy_textiles(self):
        result = delta_e_cie1994(self.color1, self.color2, K_1=0.048, K_2=0.014, K_L=2)
        expected = 1.204
        self.assertAlmostEqual(
            result,
            expected,
            3,
            "DeltaE CIE1994 (textiles) formula error. "
            "Got %.3f, expected %.3f (diff: %.3f)."
            % (result, expected, result - expected),
        )
    ...
    def test_non_lab_color(self):
        ...
        self.assertRaises(ValueError, delta_e_cie2000, self.color1, other_color)
